...
export function voiceForMood(mood: string): VoiceShape {
  const chat = loadShape().chat;
  const play = loadShape().play;
  const richer = chat * 0.06;
  const quicker = play * 0.05;
  const base = (() => {
  ...
  return {
    speed: base.speed * (1 + quicker - richer * 0.4),
    pitch: base.pitch,
    warmth: Math.min(1, base.warmth + richer),
  };
}

/** Thinking gets pauses. Startled lines stay clipped. Chat-grown speech keeps more of the line. */
export function shapeLine(text: string, mood: string): string {
  ...
  if (chat < 0.22) return line.slice(0, 56);
  return line;
}
